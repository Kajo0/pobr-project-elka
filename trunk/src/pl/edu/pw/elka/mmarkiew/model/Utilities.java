...
//					tmp[x][y] = new Pixel(255, 255, 255);
	...
	public static Pixel[][] blurPixels(final Pixel[][] pixels) {
		...
		for (int y = 0; y < height; ++y)
			for (int x = 0; x < width; ++x) {
				...
				long red = pixels[x - 1][y - 1].r * blurMask[0][0]
						+ pixels[x][y - 1].r * blurMask[0][1]
						+ pixels[x + 1][y - 1].r * blurMask[0][2]
						+ pixels[x - 1][y].r * blurMask[0][0] + pixels[x][y].r
						* blurMask[0][1] + pixels[x + 1][y].r * blurMask[0][2]
						+ pixels[x - 1][y + 1].r * blurMask[0][0]
						+ pixels[x][y + 1].r * blurMask[0][1]
						+ pixels[x + 1][y + 1].r * blurMask[0][2];
				long green = pixels[x - 1][y - 1].g * blurMask[0][0]
						+ pixels[x][y - 1].g * blurMask[0][1]
						+ pixels[x + 1][y - 1].g * blurMask[0][2]
						+ pixels[x - 1][y].g * blurMask[0][0] + pixels[x][y].g
						* blurMask[0][1] + pixels[x + 1][y].g * blurMask[0][2]
						+ pixels[x - 1][y + 1].g * blurMask[0][0]
						+ pixels[x][y + 1].g * blurMask[0][1]
						+ pixels[x + 1][y + 1].g * blurMask[0][2];
				long blue = pixels[x - 1][y - 1].b * blurMask[0][0]
						+ pixels[x][y - 1].b * blurMask[0][1]
						+ pixels[x + 1][y - 1].b * blurMask[0][2]
						+ pixels[x - 1][y].b * blurMask[0][0] + pixels[x][y].b
						* blurMask[0][1] + pixels[x + 1][y].b * blurMask[0][2]
						+ pixels[x - 1][y + 1].b * blurMask[0][0]
						+ pixels[x][y + 1].b * blurMask[0][1]
						+ pixels[x + 1][y + 1].b * blurMask[0][2];
	...
	public static BufferedImage createImageFromPixels(final Pixel[][] pixels) {
		...
		Image img = Toolkit.getDefaultToolkit().createImage(
				new MemoryImageSource(pixels.length, pixels[0].length, tmp, 0,
						pixels.length));
